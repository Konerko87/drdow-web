...
export const metadata: Metadata = createMetadata({
  title: '價格方案 — 物流 TMS / ERP 系統',
  description: 'Dr.Dow AI 物流派車系統和財務系統的價格方案。基本版、專業版、企業版，依公司規模彈性選擇。',
  path: '/pricing',
})
      ...
      <section className="pt-32 pb-16 bg-gradient-to-b from-surface to-white">
        <div className="max-w-4xl mx-auto px-6 text-center">
          <FadeIn>
            ...
            <p className="text-lg text-muted">依公司規模選擇最適合的方案，所有方案都包含 30 天免費試用。</p>
          </FadeIn>
        </div>
      </section>
            ...
            {PLANS.map((plan, i) => (
              <FadeIn key={i} delay={i * 100}>
                <div className={`rounded-2xl p-8 h-full flex flex-col ${
                  plan.featured
                    ? 'gradient-border shadow-lg shadow-accent/10'
                    : 'bg-surface'
                }`}>
                  {plan.featured && (
                    <span className="inline-block self-start text-xs font-bold text-white bg-accent px-3 py-1 rounded-full mb-4">
                      最多人選擇
                    </span>
                  )}
