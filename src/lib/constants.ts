...
export const PRODUCTS = {
  tms: {
    ...
    features: [
      { icon: 'truck', title: '智慧派車板', desc: '拖拉指派、自動填滿、表格模式三種操作。按地區分組、容量即時計算。' },
      { icon: 'smartphone', title: '司機 LINE App', desc: '不用裝 App，直接在 LINE 裡操作。任務清單、GPS 打卡、拍照回報。' },
      { icon: 'wallet', title: '薪酬自動計算', desc: '北區/中南區/轉運三種計費邏輯。保底薪資、客戶規則全自動。' },
      { icon: 'satellite', title: 'GPS 即時追蹤', desc: '每 2 分鐘更新車輛位置。軌跡回放、速度監控、地理圍欄。' },
      { icon: 'wrench', title: '車隊維護管理', desc: '保養排程、驗車提醒、報修 AI 解析。維修/耗材/保養分類。' },
      { icon: 'bar-chart', title: '營運分析 KPI', desc: '裝載率、準時率、每板成本、車輛 TCO、司機績效。10+ 維度分析。' },
      { icon: 'package', title: 'WMS 自動串接', desc: '每日自動爬取訂單。智慧路線規劃、跨區轉運拆單。' },
      { icon: 'container', title: '棧板全程追蹤', desc: '三倉庫存即時同步。司機出板/入板/歸還全紀錄。' },
      { icon: 'link', title: 'ERP 財務串接', desc: 'Webhook 雙向即時串接，SSO 統一身份。' },
    ],
  },
  erp: {
    name: 'Dr.Dow ERP',
    ...
    features: [
      { icon: 'camera', title: 'AI 拍照請款', desc: '廠商拍照上傳發票，AI 自動辨識廠商、金額、明細，準確率 95%+。' },
      { icon: 'landmark', title: '銀行自動對帳', desc: '每天 6:00 自動登入企業網銀，AI 辨識驗證碼，抓取明細自動比對。' },
      { icon: 'briefcase', title: '老闆行動 App', desc: 'iOS 風格手機介面，收款進度、一鍵審核、薪資排行、油料總覽。' },
      { icon: 'shield', title: '六層付款防呆', desc: 'DB 約束、照片 hash、軟警告、Boss 審核、API 冪等性、逐筆確認。' },
      { icon: 'store', title: '廠商自助 Portal', desc: 'LINE 登入，廠商自行送請款單、追蹤審核進度、查看撥款狀態。' },
      { icon: 'fuel', title: '加油卡即時串接', desc: '鉅泰加油卡 API，自動同步加值金額、車輛油耗排行。' },
      { icon: 'file-bar-chart', title: '財務報表', desc: '月度損益、AP/AR 帳齡分析、預算控管、Excel 匯出。' },
      { icon: 'banknote', title: '薪資管理', desc: 'CSV 上傳 + TMS 司機自動同步，勞健保勞退一目了然。' },
    ],
  },
  wms: {
    name: 'Dr.Dow WMS',
    ...
    features: [
      { icon: 'scan-barcode', title: '入庫掃描', desc: '到貨用條碼/QR 掃進系統，自動分配儲位與棧板，紙本驗收一次取代。' },
      { icon: 'truck', title: '出庫揀貨', desc: '揀貨清單按儲位排序，掃碼複核防錯出。出貨完成自動通知 TMS 派車。' },
      { icon: 'bar-chart', title: '庫存即時同步', desc: '每一次異動立刻反映可用量、安全庫存、在途數，不用等夜批。' },
      { icon: 'map', title: '儲位管理', desc: '區/排/層/位四層結構，視覺化儲位地圖，熱區/冷區一眼判讀。' },
      { icon: 'clipboard-list', title: '盤點任務', desc: '日盤、週盤、年盤任務派發，手機掃碼盤點，差異報表自動產出。' },
      { icon: 'boxes', title: '棧板與循環容器', desc: '棧板/籠車序號全程追蹤，三倉之間流轉與司機歸還一筆不漏。' },
      { icon: 'siren', title: '異常告警', desc: '揀錯、超揀、過期品、低安全庫存即時推播給倉管，事後不用補火。' },
      { icon: 'link', title: 'TMS / ERP 串接', desc: '出貨單推 TMS 派車、收貨/領料推 ERP 帳務，三系統同一筆事實來源。' },
    ],
  },
  miaotong: {
    name: 'Dr.Dow 廟通',
    ...
    features: [
      { icon: 'hand-heart', title: '信徒管理', desc: '完整信徒與戶籍資料管理，家庭關係、服務紀錄、捐款歷程一目了然。' },
      { icon: 'lamp', title: '點燈牌位服務', desc: '點燈、安太歲、牌位、進塔一站整合。受益人、燈位、期間自動管理。' },
      { icon: 'wallet', title: '捐款收據管理', desc: '具名 / 匿名捐款即時入帳，收據自動產生。日結、對帳一次搞定。' },
      { icon: 'clipboard-list', title: '法會活動管理', desc: '法會報名、繳費、桌次安排、QR Code 報到全整合。' },
      { icon: 'coins', title: '發財金借還', desc: '擲筊流程數位化，借金規則、額度、黑名單、還金追蹤全系統管理。' },
      { icon: 'bar-chart', title: '會計財務報表', desc: '收支管理、傳票、會計科目、預算控管、審核簽核。' },
      { icon: 'smartphone', title: 'LINE 行動服務', desc: '信眾免下載 App，直接用 LINE 線上點燈、法會報名、捐款查詢。' },
      { icon: 'lock', title: '角色權限與稽核', desc: '依角色設定操作權限，所有異動留稽核日誌。' },
    ],
  },
} as const
...
export const FEATURE_MODULES = [
  { icon: 'hand-heart', name: '信徒管理' },
  { icon: 'lamp', name: '點燈管理' },
  { icon: 'clipboard-list', name: '法會活動' },
  { icon: 'receipt', name: '捐款收據' },
  { icon: 'coins', name: '發財金' },
  { icon: 'folder', name: '文件中心' },
  { icon: 'bar-chart', name: '會計帳務' },
  { icon: 'handshake', name: '志工排班' },
  { icon: 'user', name: '人事管理' },
  { icon: 'trending-up', name: '報表分析' },
  { icon: 'settings', name: '系統設定' },
  { icon: 'smartphone', name: 'LINE 行動端' },
] as const
...
export const WORKFLOW_STEPS = [
  { icon: 'package', title: 'WMS 匯入', desc: '自動爬取訂單' },
  { icon: 'route', title: 'AI 路線規劃', desc: '智慧拆單分區' },
  { icon: 'truck', title: '拖拉派車', desc: '一鍵指派司機' },
  { icon: 'smartphone', title: 'LINE 執行', desc: '司機即時接單' },
  { icon: 'wallet', title: '薪酬結算', desc: 'AI 自動計算' },
  { icon: 'landmark', title: '銀行對帳', desc: '爬蟲自動比對' },
] as const
